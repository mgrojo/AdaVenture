...
--function loadshaders return interfaces.c.unsigned is separate;
...
--put_line("tex: verts'length = "&int'image(verts'length)); --120
	...
	gl.binding.glblendfunc(gl_one_minus_dst_color, gl_one_minus_src_alpha);--contrast
	elsif mode=4 then
	gl.binding.glblendfunc(gl_constant_color, gl_one_minus_src_alpha);--?
	elsif mode=5 then
	gl.binding.glblendfunc(gl_one_minus_constant_color, gl_one_minus_src_alpha);--??
	end if;
...
begin
	printex( text, xpos, ypos, size, mode );
end print2d;
...
begin
	...
	printex(text, xpos, ypos, nusize, mode);
