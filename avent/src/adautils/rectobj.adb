...
package body rectobj is -- for textured rectangular exterior, eg. partition wall
...
-------- end insert ---------------------------------------
	...
	for i in 0..5 loop
		j := i*8;
		if i=0 or i=2 then --fix Z
			umax:= xr;
			vmax:= yr;
		elsif i=1 or i=3 then --fix Y
			umax:= xr;
			vmax:= zr;
		else --fix X
			umax:= zr;
