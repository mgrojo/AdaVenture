...
-------- end insert ---------------------------------------
	...
	for i in 0..5 loop
		j := i*8;
			...
			--rect.txuv(j+1):=0.0;  rect.txuv(j+2):=1.0;
			--rect.txuv(j+3):=1.0;  rect.txuv(j+4):=1.0;
			--rect.txuv(j+5):=1.0;  rect.txuv(j+6):=0.0;
			--rect.txuv(j+7):=0.0;  rect.txuv(j+8):=0.0;
...
procedure ldraw( rect: pictangle; vertbuff, uvbuff, normbuff, elembuff : gluint ) is
begin
