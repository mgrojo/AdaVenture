-- $Id: png_io-write_png_type_3.adb,v 1.8 2016/01/10 20:53:32 sangwine Exp $
---------------------------------------------------------------------
---------------------------------------------------------------------
...
procedure Write_PNG_Type_3(Filename  : in String;
  ...
  BD : constant array(1 .. 256) of Depth range One .. Eight := ( 1 ..   2 => One,
                                                                 3 ..   4 => Two,
                                                                 5 ..  16 => Four,
                                                                17 .. 256 => Eight);
...
begin
  ...
  begin
    for Z in 0 .. Stream_Element_Offset(L - 1) loop
      Buffer(1 + 3 * Z .. 1 + 3 * Z + 2) := Stream_Element(Palette_R_Value(P, Palette_Index(Z)))
                    ...
                    for O in 0 .. Offset(Bit_Depth) loop
                      declare
                        ...
                        Col : constant Coordinate :=
                                       Coordinate'Min(Coordinate(X - 1),
                                                      Coordinate(P - 1) * Ppb(Bit_Depth) + O);
                      begin
                        B := Shift_Left(B, Shift(Bit_Depth))
                             or (Checked_Value(Pixel_Value(I, R, Map(Col))) and Mask(Bit_Depth));
                      end;
                    end loop;
