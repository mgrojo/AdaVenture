-- $Id: png_io-write_png_type_4.adb,v 1.7 2016/01/10 20:53:32 sangwine Exp $
---------------------------------------------------------------------
---------------------------------------------------------------------
      ...
      for Row in 0 .. Coordinate(Y - 1) loop
        for Col in 0 .. Coordinate(X - 1) loop
          ...
          begin
            ...
            case Bit_Depth is
              when Eight   =>
                Scanline(P .. P + 1) := Stream_Element(Sample'Pos( Grey_Sample(I, R, C)))
                                      & Stream_Element(Sample'Pos(Alpha_Sample(I, R, C)));
              when Sixteen =>
                Scanline(P .. P + 3) := To_Buffer_2(Unsigned_16(Sample'Pos( Grey_Sample(I, R, C))))
