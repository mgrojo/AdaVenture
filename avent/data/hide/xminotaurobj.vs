...
// here, we identify body segment
	...
	if ( (id==3 ) ) { //rightleg
		...
		pos.y += 0.5*pos.y * walkingDY(10.0*mytime+onepi,direction);
	}


	// arms need to swing here, BEFORE rotation
	if ( (id==4) ) { // leftarm
		pos.z += 0.8*pos.y * walkingDZ(10.0*mytime,direction);
		pos.y += 0.4*pos.y * walkingDY(10.0*mytime,direction);
	} else if ( (id==5) ) { // rightarm
		pos.z += 0.8*pos.y * walkingDZ(10.0*mytime+onepi,direction);
		pos.y += 0.4*pos.y * walkingDY(10.0*mytime+onepi,direction);
	}
