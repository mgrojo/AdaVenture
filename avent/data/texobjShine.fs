...
//uniform int numDC;
uniform int NS = 0; // 1..14
uniform int EW = 0; // 1..14
uniform float xdc[14];
uniform float zdc[14];
...
const float fPtQuadAtt = 0.25; //1.0; 30jun19: for brighter M9 chalice
...
void main(){
	...
	//float altF = exp(-0.3 * (aPos.y+iymax) );
	...
	else if( darkness>3 )  df=0.8; //0.9:tooDark
	float asave=color.a;
	color = mix(color, night, df); // night=black
	color.a = asave;


	const float dH=0.43; //0.80=hwdoor
	const float dV=0.445; //0.45=hhdoor
	const float yC=-3.0+0.47;

	if ( (NS>0) && (NS<=14) ) {
		float dx = xdc[NS-1]-aPos.x;
		float dz = zdc[NS-1]-aPos.z;
		float dy = yC - aPos.y;
		if ((abs(dy)<dV) && (abs(dx)<dH) && (abs(dz)<dH)) {
			discard;
		}
	}

	if ( (EW>0) && (EW<=14) ) {
		float dx = xdc[EW-1]-aPos.x;
		float dz = zdc[EW-1]-aPos.z;
		float dy = yC - aPos.y;
		if ((abs(dy)<dV) && (abs(dx)<dH) && (abs(dz)<dH)) {
			discard;
		}
